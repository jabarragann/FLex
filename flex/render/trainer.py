...
from flex.render.util.util import N_to_reso, get_pred_flow, get_fwd_bwd_cam2cams, get_all_poses, L1, L2, map_train_bounds2global, compute_2d_weights, get_adjacent_test_train_idxs, inverse_pose
from flex.dataloader.ray_utils import get_ray_directions_lean, get_rays_lean, ids2pixel
from local.util import generate_rel_poses, sixD_to_mtx, mtx_to_sixD
...
class Trainer:
    def __init__(
        ...
        if self.cfg.local_models and not self.cfg.local.progressive_opt:
            ...
            self.upsample_list = np.array(copy.deepcopy(self.cfg.model.upsample_list)) * (self.image_bound[1]-self.image_bound[0])
            self.update_emptymask_list = np.array(copy.deepcopy(self.cfg.model.update_emptymask_list)) * self.num_frames
            self.cfg.model.time_grid_init = round(self.num_frames/8)
            self.cfg.model.time_grid_final = round(self.num_frames/2)
            self.n_iters = round(copy.deepcopy(self.cfg.optim.n_iters) * self.num_frames)
            ...
            self.update_emptymask_list = copy.deepcopy(self.cfg.model.update_emptymask_list)
            self.n_iters = copy.deepcopy(self.cfg.optim.n_iters)
            self.refine_model = True
            print("Upsampling List: "+str(self.upsample_list))
            print("Max. iters for model: "+str(self.n_iters))

        self.all_poses, self.train_images_idxs = get_all_poses(train_dataset, test_dataset, self.total_frames)
        self.prev_test_train_idxs, self.next_test_train_idxs = get_adjacent_test_train_idxs(self.train_images_idxs, self.total_frames)
        ...
        if self.cfg.local_models:
            if self.cfg.local.progressive_opt:
                ...
                self.image_bounds = []
                self.poses_rot, self.poses_t = torch.nn.ParameterList(), torch.nn.ParameterList()
                ...
                if self.cfg.local.use_preprocessed_poses:
                    self.rel_poses, self.depth_scale = generate_rel_poses(self.all_poses, self.device)
                    # adjust depth
    ...
    def append_frame(self, initial=False):
        ...
        if torch.where(torch.tensor(self.train_images_idxs)==(self.active_frames_bounds[1]-1))[0].numel()>0:
            self.image_bound[1] += 1
        ...
        for i in range((self.active_frames_bounds[1]-old_frames_bound)):
            if len(self.poses_rot) == 0:
                ...
                self.poses_rot.append(mtx_to_sixD(sixD_to_mtx(self.poses_rot[-1].clone().detach()[None]))[0])
                self.poses_t.append(self.poses_t[-1].clone().detach())

                if self.cfg.local.use_camera_momentum:
                    last_last_pose = self.get_cam2world(img_idxs=[len(self.poses_rot)-3])
                    last_last_pose = torch.cat((last_last_pose[0], torch.tensor([0,0,0,1]).reshape(1,-1).to(self.device)),dim=0)
                    last_pose = self.get_cam2world(img_idxs=[len(self.poses_rot)-2])
                    #print(f"last pose: {self.get_cam2world()[-1]}")
                    last_pose = torch.cat((last_pose[0], torch.tensor([0,0,0,1]).reshape(1,-1).to(self.device)),dim=0)
                    last_rel_pose = np.linalg.inv(last_last_pose.detach().cpu().numpy()) @ last_pose.detach().cpu().numpy()

                    last_r_c2w = sixD_to_mtx(self.poses_rot[-1].clone().detach()[None])[0]
                    self.poses_rot[-1] = mtx_to_sixD((last_r_c2w @ torch.tensor(last_rel_pose[:3, :3]).to(self.device)))
                    self.poses_t[-1].data += last_r_c2w @ torch.tensor(last_rel_pose[:3, 3]).to(self.device)
            ...
            self.optimizers_Rot.append(torch.optim.Adam([self.poses_rot[-1]], betas=(0.9, 0.99), lr=self.cfg.optim.lr_R_init)) 
            self.optimizers_T.append(torch.optim.Adam([self.poses_t[-1]], betas=(0.9, 0.99), lr=self.cfg.optim.lr_T_init)) 
    ...
    def get_cam2world(self, img_idxs=None, initial_pose=None):
        ...
        if img_idxs is not None:
            poses_rot = torch.stack([self.poses_rot[img_idx] for img_idx in img_idxs], dim=0)
            poses_t = torch.stack([self.poses_t[img_idx] for img_idx in img_idxs], dim=0)
        else:
    ...
    def update_optimizers(self):
        if not self.refine_model:
            ...
            self.total_num_frames = (self.active_frames_bounds[1]-self.active_frames_bounds[0])
            #self.num_frames = (self.image_bound[1]-self.image_bound[0])
            self.n_iters = int(self.cfg.optim.n_iters * self.total_num_frames) # includes test frames for now
            
            # increase iters amount to adjust for test view optim when optimising poses
            
            if self.cfg.optim.optimize_poses:
                self.n_iters = int(self.n_iters * (self.active_frames_bounds[1]/self.image_bound[1]))
            
            
            lr_factor = self.get_lr_decay_factor(self.local_iteration[-1])
            self.upsample_list = np.array(copy.deepcopy(self.cfg.model.upsample_list)) * self.total_num_frames
            self.update_emptymask_list = np.array(copy.deepcopy(self.cfg.model.update_emptymask_list)) * self.total_num_frames

        if self.optimize_poses:
            for idx in range(len(self.optimizers_Rot)):
                if self.pose_linked_hex[idx] == len(self.local_iteration) - 1 and self.local_iteration[-1] < self.n_iters:
    ...
    def check_new_model(self, total_iteration, last_add_iter, train_dataset):
        ...
        if self.refine_model:
            ...
            should_refine = (not frames_left or (
                self.n_added_frames > self.cfg.local.n_overlap and (
                    dist_to_last_model > self.cfg.local.max_drift)
                    or (self.image_bound[1]-self.image_bound[0]) >= self.cfg.local.n_max_frames
                    #or abs(get_angle(last_pose, self.world2hexs[-1].clone())) >= self.cfg.local.angle_threshold # need rotation of world2hex
                ))

            if should_refine and (total_iteration - last_add_iter) >= self.cfg.local.add_frames_every:
                self.refine_model = True
                self.num_frames = (self.active_frames_bounds[1]-self.active_frames_bounds[0])
                self.n_iters = int(self.cfg.optim.n_iters * self.num_frames) # includes test frames for now
                ...
                final_time_grid_ratio = self.cfg.model.time_grid_final / self.cfg.local.n_max_frames
                self.final_time_grid = round(final_time_grid_ratio * num_train_frames)

                print(f"Start refining model {len(self.local_iteration)} for {self.n_iters} iterations and final time grid num: {self.final_time_grid}")

            elif ((total_iteration - last_add_iter) >= self.cfg.local.add_frames_every) or (not self.cfg.optim.optimize_poses and self.cfg.local.add_frames_every==0):
                self.append_frame()
                last_add_iter = total_iteration
                print("added_frame")
                print(f"Global frame bounds: {self.active_frames_bounds}, Training bounds: {self.image_bound}")
    ...
    def create_new_model(self, total_iteration, train_dataset):
        ...
        self.refine_model = False
        n_overlap = min(self.n_added_frames, self.cfg.local.n_overlap, self.blending_weights.shape[0] - 1)
        ...
        torch.save(self.hexplanes[-1], f"{self.logfolder}/{self.cfg.expname}_{(len(self.local_iteration)-1)}.th")
        ...
        hexplane, reso_cur = init_model(self.cfg, aabb, train_dataset.near_far, self.device)
        ...
        torch.save(torch.stack(list(self.poses_rot), 0), f"{self.logfolder}/{self.cfg.expname}_poses_rot.th")
        torch.save(torch.stack(list(self.poses_t), 0), f"{self.logfolder}/{self.cfg.expname}_poses_t.th")
        # save image bounds and world2hex coordinates:
        torch.save(torch.tensor(self.image_bounds), f"{self.logfolder}/{self.cfg.expname}_image_bounds.th")
        torch.save(torch.stack(list(self.world2hexs),0), f"{self.logfolder}/{self.cfg.expname}_world2hexs.th")
        # save blending weights:
        torch.save(torch.stack(list(self.blending_weights),0), f"{self.logfolder}/{self.cfg.expname}_blending_weights.th")
    ...
    def sample_data(self, train_dataset, test_dataset, iteration):
        ...
        if self.cfg.data.datasampler_type == "rays":
            ...
            if self.optimize_poses: # currently only functional for local models
                ...
                if train_test_poses:
                    ...
                    active_test_frames_idxs = actual_test_frames[torch.logical_and(torch.tensor(actual_test_frames)>=self.active_frames_bounds[0], torch.tensor(actual_test_frames)<self.active_frames_bounds[1])]
                    # map to test dataset idxs
                    active_test_frames_idxs = active_test_frames_idxs//8
                    ray_idx = torch.randint(int(active_test_frames_idxs[0]*W*H), int((active_test_frames_idxs[-1]+1)*W*H), (self.cfg.optim.batch_size,), dtype=torch.int64)
                    img_idxs = ray_idx // (W*H) # map from frame times to img idxs
                    i, j = ids2pixel(W, H, ray_idx)
                    directions = get_ray_directions_lean(i, j, [test_dataset.focal, test_dataset.focal], [test_dataset.cx, test_dataset.cy])
                    ...
                    if self.depth_data:
                      train_depth = test_dataset.all_depths.view(-1, 1)[ray_idx].to(self.device)
                    if self.train_dataset.load_tool_mask:
                        tool_mask = test_dataset.all_tool_masks.view(-1, 1)[ray_idx].to(self.device)
                    if self.cfg.data.flow_data:
                        gt_fwd = test_dataset.all_fwd.view(-1, 2)[ray_idx].to(self.device)
                        gt_bwd = test_dataset.all_bwd.view(-1, 2)[ray_idx].to(self.device)
                        fwd_mask = test_dataset.all_fwd_mask.view(-1, 1)[ray_idx].to(self.device)
                        bwd_mask = test_dataset.all_bwd_mask.view(-1, 1)[ray_idx].to(self.device)
                else:
                    ray_idx = self.sampler.nextids()
                    if self.cfg.local_models:
                        ray_idx += (self.image_bound[0]*W*H)
                    data = train_dataset[ray_idx]
                    train_img_idxs = ray_idx // (W*H) # map from frame times to train img idxs
                    global_img_idxs = self.train_images_idxs[train_img_idxs] #map from train img idxs to global img idxs
                    img_idxs = train_img_idxs
                    i, j = ids2pixel(W, H, ray_idx)
                    directions = get_ray_directions_lean(i, j, [train_dataset.focal, train_dataset.focal], [train_dataset.cx, train_dataset.cy])
                    rgb_train, frame_time = (
        ...
        # sample images: randomly pick one image from the training dataset and sample a batch of rays from all the rays of the image.
            ...
            if self.cfg.local_models:
                ...
                active_test_mask = self.test_mask
            if self.cfg.local_models and self.cfg.local.progressive_opt and self.optimize_poses:
                test_ratio = active_test_mask.mean()
                train_test_poses = test_ratio > random.uniform(0, 1)
            else:
                train_test_poses = False
            if train_test_poses:
                ...
                # map to test dataset idxs
                active_test_frames_idxs = active_test_frames_idxs//8
                img_idxs = torch.randint(0, len(active_test_frames_idxs), (self.cfg.optim.num_s_imgs,), dtype=torch.int64)

                if not self.refine_model: # Local Optimization
                    if self.cfg.optim.num_s_imgs > 5 and len(active_test_frames_idxs) > 4:
                ...
                W, H = train_dataset.img_wh
                select_inds = torch.randperm(int(H*W))[
                ...
                cam2world = self.get_cam2world(global_img_idxs.reshape(self.cfg.optim.num_s_imgs,1))
                cam2hex = cam2world.clone().detach() # do not detach when using gradients for pose optim
                cam2hex[:, :3, 3] += self.world2hexs[-1]

                rays_train = get_rays_lean(directions.to(self.device), cam2hex.repeat_interleave(directions.shape[0]//self.cfg.optim.num_s_imgs,dim=0)).reshape(-1, 6)

                ray_idxs_repeated = torch.arange(0,self.cfg.optim.num_s_imgs).repeat_interleave(self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs,dim=0) * (W*H)
                ...
                if self.train_dataset.load_tool_mask:
                    tool_mask = data["tool_mask"].to(self.device).view(-1, 1)[select_inds]
                if self.cfg.data.flow_data:
                ...
                if self.cfg.local.progressive_opt:
                    if not self.refine_model and self.optimize_poses: # Local Optimization
                        if self.cfg.optim.num_s_imgs > 5 and (self.image_bound[-1]-self.image_bound[0]) > 4:
                    ...
                    global_img_idxs = torch.tensor(self.train_images_idxs[train_img_idxs]) #map from train img idxs to global img idxs
                    img_idxs = train_img_idxs
                    W, H = train_dataset.img_wh
                    ...
                    data = train_dataset[img_i]
                    rays_train = data["rays"]
                    ray_idxs_repeated = torch.arange(0,self.cfg.optim.num_s_imgs).repeat_interleave(self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs,dim=0).to(self.device) * (W*H)
                    
                    select_inds = torch.randperm(rays_train.shape[1]).to(self.device)[
                        : self.cfg.optim.batch_size
                    ]
                    rays_train = rays_train.view(-1, 6)[(select_inds+ray_idxs_repeated).detach().cpu()]
        ...
        # hierarchical sampling from dyNeRF: hierachical sampling involves three stages of samplings.
            ...
            if iteration <= self.cfg.data.stage_1_iteration:
                ...
                if self.cfg.data.dataset_name=='neural3D_NDC':
                    ...
                    rays_train = train_dataset.all_rays[index_i * self.cfg.data.key_f_num].view(-1, 6)
                    if self.depth_data:
                        train_depth = train_dataset.all_depths[index_i * self.cfg.data.key_f_num]
                    frame_time = train_dataset.all_times[index_i * self.cfg.data.key_f_num]
                    # Calcualte the probability of sampling each ray based on the difference of global median and local values.
                    probability = GM_Resi(
                        rgb_train, self.global_mean.view(-1, 3), self.cfg.data.stage_1_gamma
                    )
                ...
                if self.depth_data:
                    train_depth = train_depth.reshape(-1,1).to(self.device)[select_inds]
            elif (
                iteration
                ...
                if self.cfg.data.dataset_name=='neural3D_NDC':
                    ...
                    rgb_train = (
                    train_dataset.all_rgbs[index_i]
                    .view(-1, 3)
                    .to(self.device)
                    )
                ...
                if self.depth_data:
                    train_depth = train_depth.reshape(-1,1).to(self.device)[select_inds]
        return rays_train, rgb_train, frame_time, train_depth, gt_fwd, gt_bwd, fwd_mask, bwd_mask, ray_idx, img_idxs, train_test_poses, tool_mask, spec_mask
    ...
    def init_sampler(self, train_dataset):
        ...
        if self.cfg.local_models:
            ...
            if self.cfg.data.datasampler_type == "rays":
                ...
                self.sampler = SimpleSampler(len(train_dataset), self.cfg.optim.num_s_imgs)
            elif self.cfg.data.datasampler_type == "hierach":
    ...
    def train(self):
        ...
        if self.optimize_poses and not self.cfg.local.progressive_opt:
            ...
            for i in range((self.active_frames_bounds[1]-self.active_frames_bounds[0])):
                pose_rot = torch.nn.parameter.Parameter(self.all_poses[i,:3,:3], requires_grad=True)
                self.poses_rot.append(pose_rot)
                pose_t = torch.nn.parameter.Parameter(self.all_poses[i,:3,3], requires_grad=True)
                self.poses_t.append(pose_t)
                optimizer_Rot = torch.optim.Adam(
                [self.poses_rot[-1]], betas=(self.cfg.optim.beta1, self.cfg.optim.beta2), lr=self.cfg.optim.lr_R_init
                )
                self.optimizers_Rot.append(optimizer_Rot)
                optimizer_T = torch.optim.Adam(
                [self.poses_t[-1]], betas=(self.cfg.optim.beta1, self.cfg.optim.beta2), lr=self.cfg.optim.lr_T_init
                )
        ...
        while(self.training):
            ...
            if self.cfg.local_models and self.cfg.local.progressive_opt:
                create_new_model, last_add_iter = self.check_new_model(total_iteration, last_add_iter, train_dataset)
                if not self.cfg.optim.optimize_poses and self.cfg.local.add_frames_every==0:
                    while(not self.refine_model):
                        create_new_model, last_add_iter = self.check_new_model(total_iteration, last_add_iter, train_dataset)
                if create_new_model:
                    if self.active_frames_bounds[1] < self.total_frames:
                        add_last_iter, reso_cur = self.create_new_model(total_iteration, train_dataset)
                        model = self.hexplanes[-1]
                    else:
                        self.training = False
                        self.image_bounds.append(self.image_bound)
                        torch.save(model, f"{self.logfolder}/{self.cfg.expname}_{(len(self.local_iteration)-1)}.th")
                        break

            # Sample data
            rays_train, rgb_train, frame_time, depth, gt_fwd, gt_bwd, fwd_mask, bwd_mask, ray_idx, img_idxs, train_test_poses, tool_mask, spec_mask = self.sample_data(
                train_dataset, test_dataset, self.local_iteration[-1]
            )
            ...
            if (self.cfg.local.progressive_opt and not self.refine_model) or total_iteration==0:
                Temp = 1.0
                ...
                Temp = 1. / (10 ** (self.local_iteration[-1] // (self.n_iters//self.cfg.optim.temp_decline)))
                summary_writer.add_scalar(
                    "train/Temp",
                    Temp,
                    global_step=total_iteration,
            ...
            if self.cfg.model.TV_weight_app > 0:
                ...
                total_loss = total_loss + loss_tv
                summary_writer.add_scalar(
                    "train/reg_tv_app", loss_tv.detach().item(), global_step=total_iteration,
                )
            ...
            if self.cfg.model.L1_weight_app > 0:
                ...
                total_loss = total_loss + loss_l1
                summary_writer.add_scalar(
                    "train/reg_l1_app", loss_l1.detach().item(), global_step=total_iteration,
                )
            ...
            if self.cfg.model.depth_loss and self.cfg.model.urf_depth_loss_weight > 0:
                    ...
                    """
                    sigma = 10.0/self.train_dataset.original_depth_scale #0.01 # default should be ca. 1cm
                    if self.cfg.local.progressive_opt and self.refine_model:
                        sigma = sigma * self.cfg.optim.lr_decay_target_ratio ** (self.local_iteration[-1] / self.n_iters)
                    ...
                    depth = depth[:, None]
                    dists = z_vals.reshape(1, z_vals.shape[-1], 1).repeat(depth.shape[0], 1, 1)
                    ...
                    line_of_sight_loss_near = (weights - target_dist_scale * torch.exp(target_distribution.log_prob(dists - depth))) ** 2
                    ...
                    urf_depth_loss = torch.mean((expected_depth_loss + line_of_sight_loss) * depth_mask)
                    total_loss += (urf_depth_loss * self.cfg.model.urf_depth_loss_weight)


                    summary_writer.add_scalar(
                    "train/urban_rad_field_depth_loss", urf_depth_loss.detach().item(), global_step=total_iteration,
                    )
                    weights = weights.reshape(weights.shape[0], weights.shape[1])

            # Optical flow loss
            start_time_train_optical = time.time()
            if self.cfg.data.flow_data and (self.cfg.model.optical_flow_loss_weight > 0) and ((not self.refine_model) or (self.local_iteration[-1]/self.n_iters) < 0.2):
                ...
                if not train_test_poses:
                    mask_test_prev, mask_test_post = train_dataset.prev_test[img_idxs].to(self.device), train_dataset.post_test[img_idxs].to(self.device)
                    mask_test_prev = ~(mask_test_prev == 1)
                    mask_test_post = ~(mask_test_post == 1)

                    if self.cfg.data.datasampler_type=="images":
                        mask_test_prev = mask_test_prev.reshape(self.cfg.optim.num_s_imgs, 1)
                        mask_test_prev = mask_test_prev.repeat(1, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs)
                        mask_test_post = mask_test_post.reshape(self.cfg.optim.num_s_imgs, 1)
                        mask_test_post = mask_test_post.repeat(1, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs)

                    actual_img_idx = torch.tensor(self.train_images_idxs[img_idxs]).to(self.device)
                    img_idx_prev = actual_img_idx - 1
                    img_idx_post = actual_img_idx + 1

                else:
                    actual_img_idx = (img_idxs*8).to(self.device) # map to global_img_idxs
                    img_idx_prev = actual_img_idx - 1
                    img_idx_post = actual_img_idx + 1

                if self.cfg.data.datasampler_type=="rays":
                    img_idx_prev[img_idx_prev<0] = 0 # set to 0 --> note this will be filtered out based on masks anyways
                    img_idx_post[img_idx_post>self.total_frames-1] = self.total_frames-1 # set to max value --> note this will be filtered out based on masks anyways
                else:
                    img_idx_prev[img_idx_prev<=0] = 0
                    img_idx_post[img_idx_post>(self.total_frames-1)] = self.total_frames-1

                if self.cfg.local.progressive_opt:

                    progressive_mask = img_idx_post<(len(self.poses_rot)-1)
                    progressive_mask = progressive_mask.reshape(1, -1)
                    img_idx_post[img_idx_post>(len(self.poses_rot)-1)] = (len(self.poses_rot)-1) # placeholder --> need to be filtered out

                    if self.cfg.data.datasampler_type=="images":
                        progressive_mask = progressive_mask.reshape(self.cfg.optim.num_s_imgs, 1)
                        progressive_mask = progressive_mask.repeat(1, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs)
                ...
                if self.cfg.data.datasampler_type=="images":
                    fwd_mask = fwd_mask.view(self.cfg.optim.num_s_imgs, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs)
                    bwd_mask = bwd_mask.view(self.cfg.optim.num_s_imgs, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs)
                    gt_fwd = gt_fwd.view(self.cfg.optim.num_s_imgs, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs, 2)
                    gt_bwd = gt_bwd.view(self.cfg.optim.num_s_imgs, self.cfg.optim.batch_size//self.cfg.optim.num_s_imgs, 2)
                else:
                ...
                if self.cfg.model.opt_from_depth:
                    ...
                    poses_post, poses_prev = get_fwd_bwd_cam2cams(cam2world, actual_img_idx)
                ...
                if self.cfg.data.datasampler_type=="images":
                    ...
                    px_original = px_original.reshape(pts.shape[0], -1, 2).to(self.device)
                else:
                    pts = directions.to(self.device)[None,...] * depth_map[..., None].clone().detach()
                    px_original = px_original.reshape(pts.shape[0], -1, 2).to(self.device)

                pred_fwd_flow, new_px_fwd = get_pred_flow(
                    pts, px_original, poses_post, focal, [train_dataset.cx, train_dataset.cy])
                pred_bwd_flow, new_px_bwd = get_pred_flow(
                    pts, px_original, poses_prev, focal, [train_dataset.cx, train_dataset.cy])

                
                if self.cfg.model.optical_flow_loss_weight > 0:

                    flow_loss_arr = torch.sum(torch.abs(pred_bwd_flow - gt_bwd), dim=-1) * bwd_mask
                    flow_loss_arr += torch.sum(torch.abs(pred_fwd_flow - gt_fwd), dim=-1) * fwd_mask
                    flow_loss_arr[flow_loss_arr > torch.quantile(flow_loss_arr, 0.9, dim=1)[..., None]] = 0
                    flow_loss = (flow_loss_arr).mean() / ((W + H) / 2)

                    total_loss = total_loss + (flow_loss * self.cfg.model.optical_flow_loss_weight * Temp)
                    summary_writer.add_scalar("train/flow_loss", flow_loss, global_step=total_iteration)
            ...
            if self.optimize_poses and train_test_poses:
                ...
                for opt in range(len(self.optimizers_Rot)):
                    if self.pose_linked_hex[opt] == len(self.local_iteration) - 1 and self.local_iteration[-1] < self.n_iters:
            ...
            summary_writer.add_scalar("train/PSNR", PSNRs[-1], global_step=total_iteration)
            ...
            if total_iteration % self.cfg.systems.progress_refresh_rate == 0:
                print(f"Iteration = {total_iteration}, train_psnr = {float(np.mean(PSNRs)):.2f}, test_psnr = {float(np.mean(PSNRs_test)):.2f}, mse = {loss:.6f}, iters/sec: {(self.cfg.systems.progress_refresh_rate/iter_time_acc):.2f}")
                PSNRs = []
                iter_time_acc = 0.0
                ...
                for param_group in optimizer.param_groups:
                    if self.cfg.optim.lr_decay_type=="cyclic":
                        ...
                        cycle = np.floor(1 + last_epoch / (2 * self.cfg.optim.cyc_step_size))
                        x = np.abs(float(last_epoch) / self.cfg.optim.cyc_step_size - 2 * cycle + 1)
                        if self.cfg.optim.cyc_mode == 'triangular':
                            lr = base_lr + (self.cfg.optim.lr_decay_target_ratio) * np.maximum(0, (1 - x))
                        elif self.cfg.optim.cyc_mode == 'triangular2':
                            lr = base_lr + (self.cfg.optim.lr_decay_target_ratio) * np.maximum(0, (1 - x)) / float(2 ** (cycle - 1))
                        elif self.cfg.optim.cyc_mode == 'exp_range':
                            lr = base_lr + (self.cfg.optim.lr_decay_target_ratio) * np.maximum(0, (1 - x)) * (self.cfg.optim.cyc_gamma ** (
                                last_epoch))
                        param_group["lr"] = lr
                    else:
                        param_group["lr"] = param_group["lr_org"] * lr_factor
            ...
            if (
                total_iteration % self.cfg.systems.vis_every == self.cfg.systems.vis_every - 1
                and self.cfg.data.N_vis != 0
            ) or (self.local_iteration[-1]==(self.n_iters-1) and self.cfg.local.progressive_opt and self.cfg.local_models and self.refine_model):
                ...
                PSNRs_test = evaluation(
                    ...
                    num_train_images=len(self.train_images_idxs) if self.cfg.local_models else len(train_dataset.poses),
                    poses_rot=poses_R,
                    poses_t=poses_T,
                    blending_weights=self.blending_weights if self.cfg.local.progressive_opt else None,
                    hexplanes=self.hexplanes if self.cfg.local.progressive_opt else None,
            ...
            if self.local_iteration[-1] in self.update_emptymask_list and not self.cfg.model.no_emptyMask:
                if (
            ...
            if self.local_iteration[-1]%self.cfg.systems.save_ckpt_every==0 and self.local_iteration[-1]!=0:
                if self.cfg.local_models:
                    torch.save(model, f"{self.logfolder}/{self.cfg.expname}_{(len(self.local_iteration)-1)}.th")
                    if self.cfg.local.progressive_opt:
                        # Save rotation and translation matrices:
            ...
            if self.cfg.model.barf_c2f is not None and not (self.cfg.model.barf_c2f[-1]==0.0):
                if not self.refine_model:
                    model.density_regressor.progress = 0.001
                    model.app_regressor.progress = 0.001 
                    if self.cfg.model.predict_flow and self.cfg.data.flow_data and (not self.cfg.model.opt_from_depth) and (not self.cfg.model.no_sep_sflow_mlp):
                        model.sflow_regressor.progress = 0.001
                else:
                    model.density_regressor.progress = self.local_iteration[-1]/self.n_iters
                    model.app_regressor.progress = self.local_iteration[-1]/self.n_iters
                    if self.cfg.model.predict_flow and self.cfg.data.flow_data and (not self.cfg.model.opt_from_depth) and (not self.cfg.model.no_sep_sflow_mlp):
                        model.sflow_regressor.progress = self.local_iteration[-1]/self.n_iters
                weights_PE = model.density_regressor.get_PE_weights()
                summary_writer.add_scalar(
                    "train/density_fea_pe_c2f_weights", np.mean(weights_PE.detach().cpu().numpy()), global_step=total_iteration
                )
