...
def save_trajectory(trajectory: list, out_dir: str, depth_scale: float):
    ...
    i_train = np.array(
        [i for i in np.arange(int(len(trajectory))) if i not in i_test])
    ...
    with open(os.path.join(out_dir, 'own_poses_train.freiburg'), 'w') as f:
        for i in i_train:
            ...
            vec = r.as_quat()
            timestep = i
            f.write(f"{timestep} {t[0]} {t[1]} {t[2]} {vec[0]} {vec[1]} {vec[2]} {vec[3]}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, help='Provide data directory on which the poses where trained. Need for proper scaling')
    args = parser.parse_args()
    convert_flex_poses(input_dir=args.input_dir, out_dir=args.out_dir, data_dir=args.data_dir)
    print('Done!')