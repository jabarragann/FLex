...
class HexPlane_Base(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.barf_c2f = kwargs.get("barf_c2f", None)
        self.contract = kwargs.get("contract", False)
    ...
    def init_app_func(
        ...
        elif AppMode == "late_view_MLP":
            self.app_regressor = MLPRender_Fea_late_view(
                self.app_dim,
                3,
                t_pe,
                fea_pe,
    ...
    def compute_flowfeature(self, xyz_sampled, frame_time):
        pass
    ...
    def forward(
        ...
        if self.depth_loss:
            if self.contract:
                depth_map = torch.sum(weight * z_vals, -1) / rays_norm[..., 0]
            else:
