...
from core.geometry.pinhole_transforms import create_img_coords_t, project, reproject
...
def warp_frame(src_frame, depth, T, intrinsics):
    ...
    warpfield = (
        _get_warpfield(
            depth.unsqueeze(0).detach().cpu(),
            T.unsqueeze(0).detach().cpu(),
            intrinsics[0].detach().cpu(),
            img_coordinates.detach().cpu(),
        )
        .squeeze()
        .reshape(2, depth.shape[-2], depth.shape[-1])
    )
...
def warp_frame_flow(src_frame, flow):
    ...
    return (
        torch.nn.functional.grid_sample(
            src_frame.unsqueeze(0).float(),
            flow_off.permute(1, 2, 0).unsqueeze(0),
            padding_mode="border",
            mode="nearest",
        )
        .squeeze()
...
def plot_res(
    ...
    img1_w_flow_batch = [
        warp_frame_flow(img, flow) for img, flow in zip(img1_batch[:2], flow_batch)
    ]
    img1_w_pose_batch = [
        warp_frame(img, depth, pose, intrinsics)
        for img, depth, pose in zip(img1_batch, depth2_batch, pose_batch)
    ]
    grid = [
        [img1, img2, img_w, img_w2, flow_img, conf1, conf2]
        for (img1, img2, img_w, img_w2, flow_img, conf1, conf2) in zip(
            img1_batch,
            img2_batch,
            img1_w_flow_batch,
            img1_w_pose_batch,
