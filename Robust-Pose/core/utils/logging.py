    ...
    def __call__(self, scene, pose, step):
        ...
        if self.gt_trajectory is not None:
            if len(self.gt_trajectory) > step:
                ...
                rot_err_deg = np.linalg.norm(
                    R.from_matrix(rot_err).as_rotvec(degrees=True), ord=2
                )
                euler_pred = R.from_matrix(pose[:3, :3]).as_euler("zxy", degrees=True)
                euler_gt = R.from_matrix(self.gt_trajectory[step][:3, :3]).as_euler(
                    "zxy", degrees=True
                )
                log_dict.update(
                    {
...
class TrainLogger:
    ...
    def _print_training_status(self, mode):
        if not self.header:
            ...
            self._print_header()
        metrics_data = [
            self.running_loss[mode][k] for k in sorted(self.running_loss[mode].keys())
        ]
