...
def save_trajectory(trajectory: list, path: str):
    with open(os.path.join(path, "trajectory.freiburg"), "w") as f:
        for tr in trajectory:
            ...
            f.write(
                f"{tr['timestamp']} {t[0]} {t[1]} {t[2]} {vec[3]} {vec[4]} {vec[5]} {vec[6]}\n"
            )
...
def json2freiburg(json_path, outpath):
    ...
    for i, pose_elem in enumerate(pose_elem_list):
        ...
        pose[0:3, 3] = -pose[
            0:3, 3
        ]  # neg. translation as Intuitive's coordinate system is inverted wrt. OpenCV
        pose[0:3, 0:3] = pose[
            0:3, 0:3
        ].T  # invert rotation as Intuitive's coordinate system is inverted wrt. OpenCV
