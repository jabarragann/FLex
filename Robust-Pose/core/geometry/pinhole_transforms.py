...
def create_img_coords_t(
    y: int, x: int, b: int = 1, device: torch.device = torch.device("cpu")
) -> torch.Tensor:
    # create 2-D coordinates
    x_mesh = torch.linspace(0, x - 1, x, device=device).repeat(b, y, 1) + 0.5
    y_mesh = (
        torch.linspace(0, y - 1, y, device=device).repeat(b, x, 1).transpose(1, 2) + 0.5
    )
    ipts = torch.vstack(
        [
            x_mesh.flatten(),
            y_mesh.flatten(),
            torch.ones(x_mesh.flatten().shape[0], device=device),
        ]
    )
...
def homogeneous(opts: torch.Tensor):
    ...
    opts = torch.cat(
        (
            opts,
            torch.ones((n, 1, opts.shape[-1]), device=opts.device, dtype=opts.dtype),
        ),
        dim=1,
    )
    return opts
...
def transform_backward(grad_out, out, opts_grad, T):
    ...
    if T.requires_grad:
        # (I | -out_x) \in R^(3x6)
        eye = torch.eye(3, device=out.device, dtype=out.dtype).repeat(
            grad_out.shape[0], 1, 1
        )
    ...
    if opts_grad:
        ...
        grad_opts = torch.bmm(
            grad_out, T.matrix().repeat(1, m, 1, 1).view(-1, 4, 4)[:, :3, :3]
        )
...
def transform(
    opts: torch.Tensor, T: Union[SE3, LieGroupParameter], double_backward: bool = False
):
    if double_backward:
