...
parser.add_argument(
    "--radius", default=None, type=float, help="Lp-sphere or ball radius"
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if method:
        print(
            f"Prepending FC layer with a BN layer and an {method.__name__} projection layer with radius {args.radius}"
        )
        batchnorm = nn.BatchNorm1d(
            model.fc.in_features, eps=1e-05, momentum=0.1, affine=False
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    with torch.no_grad():
        ...
        print(
            " * Acc@1 {top1.avg:.4f} Acc@5 {top5.avg:.4f} mAP {map:.4f}".format(
                top1=top1, top5=top5, map=map
            )
        )
