...
from utils import save_checkpoint, test
...
def parse_args():
    ...
    parser.add_argument(
        "--batchsize", type=int, default=24, help="batch size in training"
    )
    parser.add_argument(
        "--epoch", default=60, type=int, help="number of epoch in training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--decay_rate", type=float, default=1e-4, help="decay rate of learning rate"
    )
    ...
    parser.add_argument(
        "--input_transform", default=False, help="use input transform in pointnet"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="evaluate",
        action="store_true",
        help="evaluation on test set only",
    )
    parser.add_argument(
        "--outlier_fraction",
        type=float,
        default=0,
        help="fraction of data that is outliers",
    )
    parser.add_argument(
        "--robust_type",
        dest="robust_type",
        type=str,
        default="",
        help="use robust pooling {Q, PH, H, W, TQ, ''}",
    )
    parser.add_argument(
        "--alpha", dest="alpha", type=float, default=1.0, help="robustness parameter"
    )
...
def main():
    ...
    if args.outlier_fraction > 0.0:
        ...
        print(f"Number of training set outliers per point cloud: {num_outliers}")
        for i in range(train_data.shape[0]):  # For each point cloud in the batch
            random_indices = np.random.choice(
                train_data.shape[1], num_outliers, replace=False
            )
            for j in range(num_outliers):  # For each point in outlier subset
                ...
                while np.linalg.norm(random_point) > 1.0:
                    random_point = 2.0 * np.random.rand(3) - 1.0
                train_data[i, random_indices[j], :] = (
                    random_point  # Make an outlier, uniform distribution in [-1,1]^3
                )
        # Testing set
        num_outliers = int(args.outlier_fraction * test_data.shape[1])
        print(f"Number of test set outliers per point cloud: {num_outliers}")
        for i in range(test_data.shape[0]):  # For each point cloud in the batch
            random_indices = np.random.choice(
                test_data.shape[1], num_outliers, replace=False
            )
            for j in range(num_outliers):  # For each point in outlier subset
                random_point = 2.0 * np.random.rand(3) - 1.0
                # Ensure outliers are within unit sphere:
    ...
    testDataLoader = torch.utils.data.DataLoader(
        testDataset, batch_size=args.batchsize, shuffle=False
    )
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_acc = (
            test(classifier.eval(), trainDataLoader) if args.train_metric else None
        )
        acc, map, _ = test(classifier, testDataLoader, do_map=True)
        ...
        print("\r Test {}: {:f}".format(blue("Accuracy"), acc))
        print("\r Test {}: {:f}".format(blue("mAP"), map))
        if args.train_metric:
            logger.info(f"{train_acc:f},{acc:f},{map:f}")
            print(f"\r{train_acc:f},{acc:f},{map:f}")
        else:
            logger.info(f"{acc:f},{map:f}")
            print(f"\r{acc:f},{map:f}")
