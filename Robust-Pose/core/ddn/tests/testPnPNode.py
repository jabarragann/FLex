...
p3d = torch.einsum(
    "brs,bms->bmr", (R.transpose(-2, -1), p3d_transformed - t.unsqueeze(-2))
)
p2d = xy.clone()
...
# print("Input K:\n{}".format(K))
print(f"Theta Ground-Truth:\n{theta.detach().cpu().numpy()}")
print(f"Theta Estimated:\n{y.detach().cpu().numpy()}")
...
test = gradcheck(
    DL, (p2d, p3d, w, K), eps=1e-4, atol=1e-4, rtol=1e-4, raise_exception=True
)
