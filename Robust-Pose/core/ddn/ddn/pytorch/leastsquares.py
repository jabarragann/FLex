...
class WeightedLeastSquaresFcn(torch.autograd.Function):
    ...
    def forward(
        ...
        enable_bias=True,
        inverse_mode="cholesky",
    ):
        # allocate output tensors
        B, C, T = input.shape
        assert target.shape == (B, 1, T) or target.shape == (
            1,
            1,
            T,
        ), f"{input.shape} vs {target.shape}"
        assert (
            weights is None or weights.shape == (B, 1, T) or weights.shape == (1, 1, T)
        ), f"{input.shape} vs {weights.shape}"

        inverse_mode = inverse_mode.lower()
        U_sz = (
            C + 1 if (enable_bias) else C
        )  # H = DDf/DYDY is in R^{(n+1)*(n+1)} if enable_bias; otherwise, in R^{n*n}, where n=C.
        ...
        with torch.no_grad():
            ...
            if inverse_mode == "qr":  # need to get A for A=QR
                weightedsqrt = (
                    torch.ones_like(target).view(B, -1)
                    if (weights is None)
                    else torch.sqrt(weights).view(B, -1)
                )
                weightedsqrtX = (
                    input
                    if (weights is None)
                    else torch.einsum("bnm,bm->bnm", input, weightedsqrt)
                )
                weightedsqrtT = (
                    target.view(B, -1)
                    if (weights is None)
                    else torch.einsum(
                        "bm,bm->bm", target.view(B, -1), weightedsqrt
                    ).view(B, -1)
                )
                A = torch.empty(
                    (B, U_sz, C + T), device=input.device, dtype=input.dtype
                )
                ...
                if enable_bias:
                    A[:, :C, :T] = weightedsqrtX
                    A[:, -1, :T] = weightedsqrt
                    A[:, :C, T:] = math.sqrt(beta) * torch.eye(
                        C, device=input.device, dtype=input.dtype
                    )
                    A[:, -1, T:] = torch.zeros(
                        (B, C), device=input.device, dtype=input.dtype
                    )
                else:
                    A[:, :C, :T] = weightedsqrtX
                    A[:, :C, T:] = math.sqrt(beta) * torch.eye(
                ...
                weightedX = (
                    input
                    if weights is None
                    else torch.einsum("bnm,bm->bnm", input, weights.view(B, -1))
                )
                weightedTSum = (
                    target.sum(2).view(B, 1)
                    if weights is None
                    else torch.einsum(
                        "bm,bm->b", target.view(B, -1), weights.view(B, -1)
                    ).view(B, 1)
                )
                weightedXdotT = torch.einsum(
                    "bnm,bm->bn", weightedX, target.view(B, -1)
                )

                # solve x = (A^TA)^{-1} A^T b
                if enable_bias:
                    AtA = torch.empty(
                        (B, U_sz, U_sz), device=input.device, dtype=input.dtype
                    )
                    AtA[:, -1, -1] = (
                        T if weights is None else torch.sum(weights.view(B, -1), 1)
                    )
                    AtA[:, :C, :C] = torch.einsum("bik,bjk->bij", weightedX, input) + (
                        beta * torch.eye(C, device=input.device, dtype=input.dtype)
                    ).view(1, C, C)
            ...
            bias = (
                y[:, C, 0].view(B, 1)
                if enable_bias
                else torch.zeros((B, 1), device=input.device, dtype=input.dtype)
            )
    ...
    def backward(ctx, grad_output, grad_bias):
        ...
        if inverse_mode == "qr":
            if R is None:
                ...
                if enable_bias:
                    A[:, :C, :T] = weightedsqrtX
                    A[:, -1, :T] = weightedsqrt
                    A[:, :C, T:] = math.sqrt(ctx.beta) * torch.eye(
                        C, device=input.device, dtype=input.dtype
                    )
                    A[:, -1, T:] = torch.zeros(
                        (B, C), device=input.device, dtype=input.dtype
            ...
            if L is None:
                if enable_bias:
                    ...
                    AtA[:, :C, :C] = torch.einsum("bik,bjk->bij", weightedX, input) + (
                        ctx.beta * torch.eye(C, device=input.device, dtype=input.dtype)
                    ).view(1, C, C)
                    AtA[:, :C, -1] = AtA[:, -1, :C] = torch.sum(weightedX, 2)
                else:
                    AtA = torch.einsum("bik,bjk->bij", weightedX, input) + (
        ...
        if weights is not None:
            grad_input = w[:, :C].view(B, C, 1) * torch.mul(
                weights,
                (
                    target.view(B, T)
                    - torch.einsum("bn,bnm->bm", output.view(B, C), input)
                    - bias
                ).view(B, 1, T),
            ) - torch.mul(
                weights,
                (torch.einsum("bn,bnm->bm", w[:, :C].view(B, C), input) + w_bias).view(
                    B, 1, T
                ),
            ) * output.view(
                B, C, 1
            ...
            grad_target = (
                torch.einsum("bn,bnm->bm", w[:, :C].view(B, C), weightedX)
                + w_bias * weights.view(B, T)
            ).view(B, 1, T)
            ...
            grad_weights = (
                ...
                * (torch.einsum("bn,bnm->bm", w[:, :C].view(B, C), input) + w_bias)
            ).view(B, 1, T)
        else:
            ...
            grad_target = (
                torch.einsum("bn,bnm->bm", w[:, :C].view(B, C), weightedX) + w_bias
            ).view(B, 1, T)
...
class LeastSquaresLayer(nn.Module):
    ...
    def forward(self, input, target):
        return WeightedLeastSquaresFcn.apply(
            input,
            target,
            None,
            self.beta,
            self.cache_decomposition,
            self.enable_bias,
            self.inverse_mode,
        )
...
if __name__ == "__main__":
    ...
    for inverse_mode in ["cholesky", "qr"]:
        for enable_bias in [True, False]:
            # Forward check
            print(
                f"Foward test of WeightedLeastSquaresFcn, mode: {inverse_mode}, bias: {enable_bias}..."
            )
            y, y0 = f(
                X, T1, torch.ones_like(W1), 1.0e-6, False, enable_bias, inverse_mode
            )
            if torch.allclose(
                torch.einsum("bnm,bn->bm", X, y) + y0,
                T1.view(B, T),
                atol=1.0e-5,
                rtol=1.0e-3,
            ):
            ...
            # Backward check
            print(
                f"Gradient test on WeightedLeastSquaresFcn, mode: {inverse_mode}, bias: {enable_bias}..."
            )
            test = gradcheck(
                f,
                (X, T1, W1, 1.0e-3, False, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            test = gradcheck(
                f,
                (X, T2, W2, 1.0e-3, False, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            test = gradcheck(
                f,
                (X, T1, W1, 1.0e-3, True, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            test = gradcheck(
                f,
                (X, T2, W2, 1.0e-3, True, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            print(
                f"Gradient test on (unweighted) WeightedLeastSquaresFcn, mode: {inverse_mode}, bias: {enable_bias}..."
            )
            f = WeightedLeastSquaresFcn.apply
            test = gradcheck(
                f,
                (X, T1, None, 1.0e-3, False, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            test = gradcheck(
                f,
                (X, T2, None, 1.0e-3, False, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            test = gradcheck(
                f,
                (X, T1, None, 1.0e-3, True, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
            ...
            test = gradcheck(
                f,
                (X, T2, None, 1.0e-3, True, enable_bias, inverse_mode),
                eps=1e-6,
                atol=1e-3,
                rtol=1e-6,
            )
    ...
    if (
        torch.allclose(y_true, y_false)
        and torch.allclose(y0_true, y0_false)
        and (y0_false != 0.0)
    ):
        print("Failed")
