...
class UnconstPolynomial(AbstractDeclarativeNode):
    ...
    def solve(self, x):
        ...
        y_stationary = torch.cat(
            (
                torch.zeros_like(x),
                (-3.0 * x**2.0 - delta) / (4.0 * x),
                (-3.0 * x**2.0 + delta) / (4.0 * x),
            ),
            dim=-1,
        )
        ...
        y = torch.cat(
            [
                torch.index_select(a, dim=0, index=i).unsqueeze(0)
                for a, i in zip(y_stationary, y_min_indx)
            ]
        )
        return y, None
...
class GlobalPseudoHuberPool2d(AbstractDeclarativeNode):
    ...
    def gradient(self, x, alpha, y=None, v=None, ctx=None):
        ...
        Dy_at_x = torch.where(
            w_sum.abs() <= 1e-9, torch.zeros_like(w), w.div(w_sum)
        )  # b x n1 x n2
...
class ConstLinFcnOnParameterizedCircle(EqConstDeclarativeNode):
    ...
    def equality_constraints(self, x, y):
        return torch.einsum("bm,bm->b", (y, y)) - torch.einsum(
            "b,b->b", (x[:, 0], x[:, 0])
        )
    ...
    def solve(self, x):
        y = (
            -1.0
            * torch.abs(x[:, 0]).unsqueeze(-1)
            * x.new_ones(x.size(0), 2)
            / math.sqrt(2.0)
        )  # bx2
...
class LinFcnOnParameterizedCircle(EqConstDeclarativeNode):
    ...
    def solve(self, x):
        y = (
            -1.0
            * torch.abs(x[:, 1]).unsqueeze(-1)
            * torch.cat((torch.ones_like(x[:, 0:1]), x[:, 0:1]), dim=-1)
            / torch.sqrt(1.0 + torch.pow(x[:, 0], 2.0)).unsqueeze(-1)
        )  # bx2
    ...
    def gradient(self, x, y=None, v=None, ctx=None):
        ...
        a = (
            torch.abs(x[:, 1]).unsqueeze(-1)
            * torch.cat((x[:, 0:1], -1.0 * torch.ones_like(x[:, 0:1])), dim=-1)
            / torch.pow(1.0 + torch.pow(x[:, 0], 2.0), 1.5).unsqueeze(-1)
        )  # bx2
        b = (
            -1.0
            * torch.sign(x[:, 1]).unsqueeze(-1)
            * torch.cat((torch.ones_like(x[:, 0:1]), x[:, 0:1]), dim=-1)
            / torch.sqrt(1.0 + torch.pow(x[:, 0], 2.0)).unsqueeze(-1)
        )  # bx2
...
class QuadFcnOnSphere(EqConstDeclarativeNode):
    ...
    def gradient(self, x, y=None, v=None, ctx=None):
        ...
        eye_batch = torch.eye(x.size(1), dtype=x.dtype, device=x.device).expand_as(
            x_outer
        )
        Dy_at_x = (
            torch.einsum("b,bmn->bmn", (x_inner, eye_batch)) - x_outer
        ) / torch.pow(torch.einsum("bm,bm->b", (x, x)), 1.5).unsqueeze(-1).unsqueeze(-1)
        return (torch.einsum("bm,bmn->bn", (v, Dy_at_x)),)
...
class QuadFcnOnBall(IneqConstDeclarativeNode):
    ...
    def gradient(self, x, y=None, v=None, ctx=None):
        ...
        Dy_at_x = torch.where(
            x_inner.unsqueeze(-1).unsqueeze(-1) <= 1.0,
            eye_batch,
            (torch.einsum("b,bmn->bmn", (x_inner, eye_batch)) - x_outer)
            / torch.pow(torch.einsum("bm,bm->b", (x, x)), 1.5)
            .unsqueeze(-1)
            .unsqueeze(-1),
        )
        return (torch.einsum("bm,bmn->bn", (v, Dy_at_x)),)
