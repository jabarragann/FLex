...
class RobustAverage(NonUniqueDeclarativeNode):
    ...
    def __init__(self, n, penalty="huber", alpha=1.0):
        ...
        if self.penalty == "quadratic":
            ...
            self.phi = lambda z: self.alpha_sq * (
                np.sqrt(1.0 + np.power(z, 2.0) / self.alpha_sq) - 1.0
            )
            ...
            assert False, f"unrecognized penalty function {penalty}"
    ...
    def solve(self, x):
        ...
        if (self.penalty == "welsch") or (self.penalty == "trunc-quad"):
            ...
            if len(guesses) > self.restarts:
                guesses = guesses[: self.restarts]
    ...
    def gradient(self, x, y=None, ctx=None):
        ...
        if self.penalty == "quadratic":
            ...
            dy = np.array(
                [
                    np.power(1.0 + np.power(y - xi, 2.0) / self.alpha_sq, -1.5)
                    for xi in x
                ]
            )
            ...
            dy = np.array(
                [
                    (self.alpha_sq - zi)
                    / (self.alpha_sq * self.alpha_sq)
                    * np.exp(-0.5 * zi / self.alpha_sq)
                    for zi in z
                ]
            )
...
class RobustVectorAverage(NonUniqueDeclarativeNode):
    ...
    def objective(self, x, y):
        ...
        return np.sum(
            [
                self.phi(np.sqrt(np.dot(y - xi, y - xi) + 1.0e-9))
                for xi in x.reshape((int(self.dim_x / self.dim_y), self.dim_y))
            ]
        )
    ...
    def solve(self, x):
        ...
        # run with different intial guesses for non-convex penalties
        if (self.penalty == "welsch") or (self.penalty == "trunc-quad"):
            guesses = np.random.permutation(
                x.reshape((int(self.dim_x / self.dim_y), self.dim_y))
            )
            if self.dim_x > self.restarts:
                guesses = guesses[0 : self.restarts, :]
