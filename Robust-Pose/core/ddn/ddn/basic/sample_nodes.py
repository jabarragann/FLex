...
class LinFcnOnParameterizedCircle(EqConstDeclarativeNode):
    ...
    def solve(self, x):
        y_star = (
            -1.0 * np.fabs(x[1]) / np.sqrt(1.0 + x[0] ** 2.0) * np.array([1.0, x[0]])
        )
    ...
    def gradient(self, x, y=None, ctx=None):
        ...
        return np.vstack(
            (
                np.abs(x[1]) / np.power(1 + x[0] ** 2.0, 1.5) * np.array([x[0], -1.0]),
                -1.0
                * np.sign(x[1])
                / np.sqrt(1.0 + x[0] ** 2.0)
                * np.array([1.0, x[0]]),
...
class QuadFcnOnSphere(EqConstDeclarativeNode):
    ...
    def gradient(self, x, y=None, ctx=None):
        ...
        return (
            1.0
            / np.power(np.dot(x, x), 1.5)
            * (np.dot(x, x) * np.eye(self.dim_x) - np.outer(x, x))
        )
...
class QuadFcnOnBall(IneqConstDeclarativeNode):
    ...
    def gradient(self, x, y=None, ctx=None):
        ...
        return (
            1.0
            / np.power(x_norm_sq, 1.5)
            * (x_norm_sq * np.eye(self.dim_x) - np.outer(x, x))
        )
