...
import numpy as np
import torch
from core.geometry.pinhole_transforms import (
    create_img_coords_t,
    project,
    project2image,
    reproject,
...
class SurfelMap:
    def __init__(self, *args, **kwargs):
        ...
        super().__init__()
        self.pmat = (
            kwargs["pmat"] if "pmat" in kwargs else SE3.Identity(1)
        )  # extrinsics
        ...
        self.depth_scale = (
            kwargs["depth_scale"] if "depth_scale" in kwargs else 1.0
        )  # only used for open3d pcl
        ...
        if "opts" in kwargs:
            ...
            if "conf" in kwargs:
                ...
                self.conf = torch.exp(-0.5 * gamma**2 / 0.6**2)[None, :]
        else:
            ...
            mask = (
                torch.ones_like(frame.depth).to(torch.bool)
                if ignore_mask
                else frame.mask
            )
            # calculate object points
            ipts = (
                create_img_coords_t(y=self.img_shape[-2], x=self.img_shape[-1])
                .to(self.device)
                .to(dtype)
            )
            ...
            opts = reproject(
                img_coords=ipts,
                intrinsics=self.kmat.to(self.device).to(dtype),
                depth=frame.depth,
            )
            self.opts = transform(
                opts, self.pmat.to(self.device).to(dtype)[None, ...]
            ).squeeze()[:3, mask.view(-1)]

            self.rgb = frame.img.view(3, -1)[:, mask.view(-1)]
            self.conf = (
                frame.confidence[mask].view(1, -1) / self.conf_thr
    ...
    def fuse(self, *args):
        ...
        if self.upscale > 1:
            ...
            rgb = torch.nn.functional.interpolate(
                rgb, scale_factor=self.upscale, mode="bilinear", align_corners=None
            )
            depth = torch.nn.functional.interpolate(
                depth, scale_factor=self.upscale, mode="bilinear", align_corners=None
            )
            kmat[:2] *= self.upscale
        ...
        ipts = (
            create_img_coords_t(
                y=self.img_shape[-2] * self.upscale, x=self.img_shape[-1] * self.upscale
            )
            .to(dtype)
            .to(self.device)
        )
        ...
        global_ipts = project(
            self.opts.unsqueeze(0), intrinsics=kmat.unsqueeze(0), T=pose_inv[None, ...]
        ).squeeze()
        bidx = (
            (global_ipts[0, :] >= 0)
            & (global_ipts[1, :] >= 0)
            & (global_ipts[0, :] < self.img_shape[1] * self.upscale - 1)
            & (global_ipts[1, :] < self.img_shape[0] * self.upscale - 1)
        )
        ...
        vidx, midx = self.filter_surfels_by_correspondence(
            opts=opts, vidx=bidx, midx=midx, d_thresh=self.d_thresh
        )

        # apply frame mask to reject invalid pixels
        bidx[vidx.clone()] &= (
            frame.mask.view(-1)[(midx / self.upscale**2).long()]
        ).type(torch.bool)
        ...
        if self.average_points:
            self.opts[:, vidx] = (
                conf_idx * self.opts[:, vidx] + ccor * opts[:, midx]
            ) / (conf_idx + ccor)
            self.rgb[:, vidx] = (conf_idx * self.rgb[:, vidx] + ccor * rgb[:, midx]) / (
                conf_idx + ccor
            )
        self.conf[:, vidx] = torch.clamp(
            conf_idx + ccor, 0.0, 1.0
        )  # saturate confidence to 1
        ...
        mask = (
            ~max_pool2d(
                mask.view(
                    1,
                    1,
                    self.img_shape[0] * self.upscale,
                    self.img_shape[1] * self.upscale,
                ),
                self.upscale,
                stride=self.upscale,
            )
        ...
        self.t_created = torch.cat(
            (self.t_created, self.tick * torch.ones(1, mask.sum()).to(self.device)),
            dim=-1,
        )
    ...
    def remove_surfels_by_confidence_and_time(self):
        ...
        ok_pts = (
            (self.conf >= 1.0) | ((self.tick - self.t_created) < self.t_max)
        ).squeeze()
    ...
    def _downsample(self, x):
        x = x.view(
            -1, self.img_shape[0] * self.upscale, self.img_shape[1] * self.upscale
        )
        x = x[:, :: self.upscale, :: self.upscale].reshape(x.shape[0], -1)
        return x

    def get_match_indices(
        self, ipts: torch.Tensor = None, upscale: int = None
    ) -> torch.Tensor:
        if upscale is None:
            upscale = self.upscale
        # quantize points (while considering super-sampling factor)
        ipts_quantized = torch.round(ipts - 0.5)
    ...
    def render(self, intrinsics: torch.tensor = None, extrinsics: SE3 = None):
        ...
        confidence = torch.zeros(
            self.img_shape, dtype=self.opts.dtype, device=self.device
        )
        ...
        colors = torch.zeros(
            (3, *self.img_shape), dtype=self.opts.dtype, device=self.device
        )
        ...
        return (
            Frame(
                colors, depth=depth, mask=mask, confidence=confidence[None, None, ...]
            ).to(intrinsics.device),
            None,
        )
    ...
    def pcl2open3d(self, stable: bool = True, filter: torch.Tensor = None):
        ...
        if stable:
            ...
            stable_pts = torch.ones_like(
                self.conf[:, filter], dtype=torch.bool
            ).squeeze()

        pcd.points = open3d.utility.Vector3dVector(
            self.opts.T[filter][stable_pts].cpu().numpy() / self.depth_scale
        )
        if self.rgb.numel() > 0:
