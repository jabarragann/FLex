...
class DeclarativeNodeLie(AbstractDeclarativeNode):
    ...
    def gradient(
        ...
        *xs: Tuple[torch.tensor],
        y: Tuple[torch.tensor] = None,
        v: Tuple[torch.tensor] = None,
        ctx=None
    ):
        ...
        for x_split, x_size, n in zip(xs_split, xs_sizes, self.n):
            if isinstance(x_split[0], torch.Tensor) and x_split[0].requires_grad:
                ...
                gradient[torch.isnan(gradient)] = (
                    0.0  # nan values may occur due to zero-weights
                )
    ...
    def _batch_jacobian(self, y, x, create_graph=False):
        ...
        for i in range(m):
            ...
            jacobian[:, i : (i + 1), :] = torch.cat(
                [a.reshape(self.b, 1, -1) for a in yiX], dim=-1
            )
    ...
    def _batch_jacobian_fast(self, y, x, create_graph=False):
        ...
        grad_outputs = torch.eye(
            m, requires_grad=False, dtype=y.dtype, device=y.device
        ).repeat((b, 1, 1))
...
class DeclarativeFunctionLie(DeclarativeFunction):
    ...
    * All inputs have a single batch dimension (b, ...)
            ...
            *outs, solve_ctx = problem.solve(*inputs)
            ...
            for out in outs:
                if isinstance(out, LieGroupParameter):
                    ...
                    outs_tan.append(
                        out.detach().float()
                    )  # tangent space is embedding space for vector
    ...
    def backward(ctx, *grad_outs):
        ...
        grad_outs_tan = grad_outs[
            len(out_types) :
        ]  # (embedding grad outs, tanget grad outs)
        grad_inputs = problem.gradient(
            *inputs, y=outs_param, v=grad_outs_tan, ctx=solve_ctx
        )
