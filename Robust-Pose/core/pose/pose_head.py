from core.geometry.pinhole_transforms import project, transform
...
class DPoseSE3Head(DeclarativeNodeLie):
    def __init__(self, img_coordinates, lbgfs_iters=100, dbg=False):
        super().__init__(eps=1e-3, dbg=dbg)
        self.img_coordinates = img_coordinates
        self.lbgfs_iters = lbgfs_iters
        self.losses = []

    def reprojection_objective(
        self, flow, pcl1, weights1, mask1, intrinsics, y, backward=False, ret_res=False
    ):
        """
        ...
        warped_pts = project(
            pcl1.view(n, 3, -1), intrinsics, y, double_backward=backward
        )[:, :2]
        ...
        valid = (
            torch.isinf(residuals)
            | torch.isnan(residuals)
            | ~valid.view(n, -1)
            | ~mask1.view(n, -1)
        )
    ...
    def depth_objective(
        self, pcl1, pcl2, weights2, mask1, mask2, y, backward=False, ret_res=False
    ):
        """
        ...
        pcl1_aligned = transform(
            pcl1.view(n, 3, -1), y, double_backward=backward
        ).reshape(n, 3, h, w)
        # compute residuals
        residuals = torch.sum(
            (pcl1_aligned.view(n, 3, -1) - pcl2.view(n, 3, -1)) ** 2, dim=1
        )
    ...
    def objective(self, *xs, y, backward=False):
        ...
        loss3d = self.depth_objective(
            pcl1, pcl2, weights2, mask1, mask2, pose, backward
        )
        loss2d = self.reprojection_objective(
            flow, pcl1, weights1, mask1, intrinsics, pose, backward
        )
    ...
    def solve(self, *xs):
        ...
        with torch.enable_grad():
            ...
            y = LieGroupParameter(
                SE3.Identity(
                    n, 1, device=xs[0].device, requires_grad=True, dtype=torch.float64
                )
            )
