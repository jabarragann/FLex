...
import torch
from core.fusion.surfel_map import Frame, SurfelMap
...
class PoseEstimator(torch.nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "scale", torch.tensor(1 / config["depth_clipping"][1]), persistent=False
        )
        self.last_pose = init_pose.float("api bug")
        self.register_buffer(
            "baseline", torch.tensor(baseline).unsqueeze(0).float(), persistent=False
        )
    ...
    def forward(
        self, limg: torch.Tensor, rimg: torch.Tensor, mask: torch.Tensor
        ...
        if self.frame2frame:
            ...
            if self.scene is None:
                ...
                depth, stereo_flow, valid = self.model.flow2depth(
                    self.frame.img, self.frame.rimg, self.baseline * self.scale
                )
                self.frame.depth = depth / self.scale
                self.frame.mask &= valid
                self.frame.flow = stereo_flow
                self.scene = SurfelMap(
                    frame=self.frame,
                    kmat=self.intrinsics.squeeze(),
                    upscale=1,
                    d_thresh=self.config["dist_thr"],
                    pmat=self.last_pose,
                    average_pts=self.config["average_pts"],
                ).to(self.device)
            rel_pose, ret_frame, flow, weights, depth, flow_bwd = self.get_pose_f2m()

        # check if pose is valid
        if (torch.isnan(rel_pose.vec()).any()) | (
            torch.abs(rel_pose.log()) > 1.0e-1
        ).any():
    ...
    def get_pose_f2f(self):
        ...
        if self.last_frame is None:
            ...
            depth_r, stereo_flow_r, valid_r = self.model.flow2depth(
                self.frame.rimg,
                self.frame.img,
            ...
            # get pose
            (
                (rel_pose_se3, depth1, depth2, weights, flow, stereo_flow),
                flow_bwd,
                valid,
                depth_r,
                valid_r,
