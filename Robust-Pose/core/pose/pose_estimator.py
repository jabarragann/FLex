...
class PoseEstimator(torch.nn.Module):
    ...
    def forward(self, limg: torch.Tensor, rimg: torch.Tensor, mask: torch.Tensor) \
        ...
        if self.frame2frame:
            rel_pose, ret_frame, flow, weights, depth, flow_bwd, valid, depth_r, valid_r = self.get_pose_f2f()
        else:
            ...
            rel_pose, ret_frame, flow, weights, depth, flow_bwd = self.get_pose_f2m()
        ...
        return self.last_pose, self.scene, flow, weights, depth, flow_bwd, valid, depth_r, valid_r
    ...
    def get_pose_f2f(self):
        ...
        if self.last_frame is None:
            ...
            depth_r, stereo_flow_r, valid_r = self.model.flow2depth(self.frame.rimg, self.frame.img, self.baseline*self.scale, right_to_left=True)
            self.frame.depth = depth/self.scale
            self.frame.flow = stereo_flow
            ret_frame = None
            weights = None
            ...
            # get pose
            (rel_pose_se3, depth1, depth2, weights, flow, stereo_flow), flow_bwd, valid, depth_r, valid_r = self.model.infer(self.last_frame.img, self.frame.img,
        ...
        return rel_pose_se3, ret_frame, flow, weights, self.frame.depth, flow_bwd, valid, depth_r/self.scale, valid_r
    ...
    def get_pose_f2m(self):
        ...
        # get pose
        (rel_pose_se3, depth1, depth2, weights, flow, stereo_flow), flow_bwd = self.model.infer(model_frame.img, self.frame.img,
                                                                              self.intrinsics,
                                                                              self.baseline * self.scale,
        ...
        return rel_pose_se3, model_frame, flow, weights, self.frame.depth, flow_bwd
