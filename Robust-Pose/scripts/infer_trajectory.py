...
def main(args, config):
    ...
    pose_estimator = PoseEstimator(config['slam'], torch.tensor(calib['intrinsics']['left']).to(device), baseline=calib['bf'],
                                    checkpoint=args.checkpoint, img_shape=adj_img_size, init_pose=init_pose).to(device)
        ...
        if args.metrics_only is None:
            for i, data in enumerate(tqdm(loader, total=min(len(dataset), (args.stop-args.start)//args.step))):
                ...
                if h%32!=0 or w%32!=0:
                        ...
                        print(limg.shape)
                pose, scene, flow, weights, depth, flow_bwd, valid_depth, depth_r, valid_depth_r = pose_estimator(limg.to(device), rimg.to(device), mask.to(device))

                # save flow and depth maps:
                fbase = '{}'.format(str(i).zfill(4))
                ...
                if flow is not None:
                    ...
                    cv2.imwrite(f"{args.outpath}/flow_ds/fwd/fwd_{fbase}.png", encode_flow(flow_fwd, mask_fwd))
                    cv2.imwrite(f"{args.outpath}/flow_ds/bwd/bwd_{fbase}.png", encode_flow(flow_bwd, mask_bwd))
                else:
                    flow_fwd = np.zeros(limg[0,:,:h,:w].permute(1,2,0)[..., :2].shape, dtype=np.float32)
                    flow_bwd = np.zeros(limg[0,:,:h,:w].permute(1,2,0)[..., :2].shape, dtype=np.float32)
                    mask_fwd = np.zeros(limg[0,:,:h,:w].permute(1,2,0)[..., 0].shape, dtype=bool)
                    mask_bwd = np.zeros(limg[0,:,:h,:w].permute(1,2,0)[..., 0].shape, dtype=bool)
                ...
                if h%32!=0 or w%32!=0:
                    img_name = f'{i:06d}'
                    cv2.imwrite(f"{args.outpath}/adj_video_frames/{img_name}l.png", cv2.cvtColor(limg[0].permute(1,2,0).cpu().numpy().astype(np.uint8), cv2.COLOR_BGR2RGB))
                    cv2.imwrite(f"{args.outpath}/adj_video_frames/{img_name}r.png", cv2.cvtColor(rimg[0].permute(1,2,0).cpu().numpy().astype(np.uint8), cv2.COLOR_BGR2RGB))
                    cv2.imwrite(f"{args.outpath}/adj_masks/{img_name}l.png", mask[0].permute(1,2,0).cpu().numpy())
        ...
        if args.metrics is not None:
            ...
            if os.path.isfile(os.path.join(args.input, 'groundtruth.txt')):
                ate_rmse, rpe_trans, rpe_rot, trans_error, rpe_trans_e, rpe_rot_e = eval(os.path.join(args.input, 'groundtruth.txt'),
                                 os.path.join(args.outpath, 'trajectory.freiburg'), offset=0)
