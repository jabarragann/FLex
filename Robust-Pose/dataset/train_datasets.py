...
def get_data(config: dict, img_size: Tuple, depth_cutoff: float):
    ...
    for i in range(len(config["sequences"])):
        calib_path = os.path.join(
            config["basepath"], config["sequences"][i], "keyframe_1"
        )
        ...
        rect = StereoRectifier(
            calib_file, img_size_new=(img_size[1], img_size[0]), mode="conventional"
        )
    ...
    ds = [
        sorted(glob(os.path.join(config["basepath"], s, "keyframe_*")))
...
class PoseDataset(Dataset):
    def __init__(
        ...
        for i in sample_list:
            s = (
                np.random.randint(*step) if step[0] < step[1] else step[0]
            )  # select a random step in given range

            img_number1 = int(os.path.basename(n_list[i]).split("l.png")[0])
            img_number2 = int(os.path.basename(n_list[i + s]).split("l.png")[0])
            self.image_list.append(
                [
                    n_list[i].replace("masks", "video_frames"),
                    n_list[i + s].replace("masks", "video_frames"),
                ]
            )
            self.rel_pose_list.append(
                poses[img_number1 - 1].inv().mul(poses[img_number2 - 1])
            )
            self.image_list_r.append(
                [
                    n_list[i]
                    .replace("masks", "video_frames")
                    .replace("l.png", "r.png"),
                    n_list[i + s]
                    .replace("masks", "video_frames")
                    .replace("l.png", "r.png"),
            ...
            if len(masks) == 0:
                ...
                self.mask_list.append(
                    [
                        n_list[i].replace("video_frames", "masks"),
    ...
    def __getitem__(self, index):
        ...
        pose = pose.scale(
            torch.tensor(1 / self.depth_cutoff)
        )  # scale translation for normalized depth
        baseline = float(
            self.baseline[index] / self.depth_cutoff
        )  # scale stereo-baseline for normalized depth
    ...
    def _read_img(self, path):
        img = (
            torch.from_numpy(cv2.cvtColor(cv2.imread(path), cv2.COLOR_BGR2RGB))
            .permute(2, 0, 1)
            .float()
        )
    ...
    def _random_sample(self, step, samples, total_samples):
        ...
        if (samples > 0) & (samples < total_samples):
            sample_list = sorted(
                np.random.choice(
                    total_samples - step[1], size=(samples,), replace=False
                )
            )
        else:
