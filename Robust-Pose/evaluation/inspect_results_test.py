...
parser = argparse.ArgumentParser(
    description="Inspect WandB results of test set benchmarking"
)
parser.add_argument(
    "project",
    type=str,
...
for run in runs:
    ...
    all_dict.update({k: v for k, v in run.config.items() if not k.startswith("_")})
...
for method in METHODS:
    ...
    df = pd.DataFrame(
        {
            "mean": df.groupby("dataset").mean()["ATE/RMSE"],
            "std": df.groupby("dataset").std()["ATE/RMSE"],
        }
    )
    print(df)
...
for method in METHODS:
    ...
    df = pd.DataFrame(
        {
            "mean": df.groupby("dataset").mean()["RPE/trans"],
            "std": df.groupby("dataset").std()["RPE/trans"],
        }
    )
    print(df)
    print("macro average:", df.mean()["mean"], "+/-", df.std()["mean"])
...
for method in METHODS:
    ...
    df = pd.DataFrame(
        {
            "mean": df.groupby("dataset").mean()["RPE/rot"],
            "std": df.groupby("dataset").std()["RPE/rot"],
        }
    )
    print(df)
    print("macro average:", df.mean()["mean"], "+/-", df.std()["mean"])
